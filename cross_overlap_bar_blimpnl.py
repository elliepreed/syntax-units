...
def main():
    ...
    # Put categories with valid within-category values first, singleton categories last.
    ...
    max_val = np.nanmax(plot_df[["within", "outbound"]].to_numpy(dtype=float))
    x_max = min(100, max(10, np.ceil((max_val + 8) / 10) * 10))
    ax.set_xlim(0, x_max)
